...
\begin{frame}{Liens avec les graphes planaires}
    \begin{cor}
        Les graphes planaires forment une sous classe stricte des graphes de carte.
    \end{cor}

    \begin{proof}
        Tout graphe planaire admet un témoin, il suffit de subdiviser une fois toutes les arrêtes du graphe planaire pour
        en obtenir un. Il existe des graphes de carte non planaires, par exemple les grands graphes complets.
    \end{proof}
\end{frame}

\begin{frame}{Quelques propriétés}
    \begin{proposition}[Hérédité]
    ...
    On considèrera par la suite que tout les graphes étudiés sont $2$-connexes.
\end{frame}
...
\begin{frame}{Quelques résultats prometteurs}
    ...
    \begin{thm}[Angelin, Bekos, Da Lozzo, Gronemann, Montecchiani, Tappini, 2024]
        Le problème de reconnaissance paramétré par la largeur arborescente est FPT
    \end{thm}
\end{frame}
...
\begin{frame}
    On dénotera par $G_{ex}$ un graphe à $3$ sommets n'ayant qu'une seule arrête.
    \begin{figure}
        \caption{Le join d'un indépendant à $2$ sommets avec $G_{ex}$}
