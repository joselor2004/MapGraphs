...
\begin{lem}
    Un cographe $G$ est connexe si et seulement si son coarbre $T$ a pour racine un noeud étiquetté $1$
\end{lem}
...
\begin{proof}
    Les ensembles décrits sont bien des séparateurs minimaux : notons $\displaystyle S_j = \bigcup_{i \neq j} V(T_i)$ pour un tel $j$. $G - S_j$ peut être
    ...
    \\~\\
    Réciproquement, soit $S$ un séparateur. Soient $s, t$ deux sommets séparés par $S$. Le plus petit ancêtre commun de $s$ et $t$ dans $T$
...
On a un résultat ULTRA restricif sur les $G$ de carte $3$-connexe. Peut être qu'on peut le généraliser en parlant de sous arbres
d'un arbre maximal ??

\begin{lem}
    Soit $G$ un cographe $3$-connexe. $G$ est de carte si et seulement si il s'agit de l'un des graphes suivants,
    $p, q, r, s \geq 1, n \geq 0$, et où les valeurs minimales des indices dépendent du cas, de manière ce que $G$ soit $3$-connexe :
    \begin{itemize}
        \item $K(2,2, K_p \sqcup K_q, K_n)$
        \item $K(K_p \sqcup K_q, K_r \sqcup K_s, K_n)$
        \item $K(K_p \sqcup K_q, K_n)$ avec $n \geq 3$
        \item $K_n$, $n \geq 1$ 
    \end{itemize}
\end{lem}

\begin{proof}
    On distingue selon le nombre de fils de la racine étiquettés $0$
\end{proof}
