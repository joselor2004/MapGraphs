...
\begin{center}
    Stage réalisé au LIMOS, Clermont-Ferrand, encadré par Vincent Limouzy
\end{center}
...
est un sommet adjacent à tous les sommets du graphe.
...
On pourrait alors se demander, à la lumière des exemples précédents si un graphe n'étant pas $3$-connexe
...
n'ayant pas ces $3$ sous graphes induits sera de carte car tout le raisonnement est basé sur le seul Lemme \ref{trivPerfind}. On peut
déduire de ces résultats structurels le résultat algorithmique suivant

\begin{theorem}
    Il existe un algorithme prenant en entrée un graphe trivialement parfait $G$ et retournant en temps polynômial, un témoin de $G$
    si ce dernier est de carte, ou un sous graphe induit $K(3, K_3)$ le cas échéant.    
\end{theorem}

\begin{proof}
    On suppose sans perte de généralité $G$ $2$-connexe.\\
    L'algorithme est le suivant : dans le cas où $G$ est $3$-connexe, il suffit d'identifier si il est de l'une des formes décrite
    par le Lemme \ref{trivPar3conn}. Pour cela, on énumère ses cliques max : si il n'en a qu'une, $G$ est une clique, si il en a
    deux, alors $G$ est de la forme $K(K_n, I_{p,q})$. On construit alors des témoins en s'inspirant de la carte Figure \ref{KKnIpqmap}.
    Si $G$ a plus de $3$ cliques max, il possède un indépendant de taille $3$ car $G$ ne peut être de carte, n'étant pas de la bonne
    forme. Il suffit alors d'en trouver un et de compléter par les $3$ universels pour obtenir un sous graphe induit $K(3, K_3)$.
    \\~\\
    Si $G$ est uniquement $2$-connexe, on vérifie d'abord que ses composantes $3$-connexes sont de carte. Si l'une ne l'est pas on renvoie le
    $K(3, K_3)$ trouvé pour cette dernière. Sinon, on construit un témoin à $G$ en s'inspirant de la carte construite lors du
    Théorème \ref{maptrivperf}.
    \\~\\
    Toutes les opérations décrites se font en temps polynômial.
\end{proof}
...
\begin{proof}
    ...
    $G$ étant de carte, $G_i$ ne peut être de type au moins $2$ :
    en effet le noeud $0$ ayant pour sous arbres les composantes de $G - \{x,y\}$ admet un sommet descendant
...
à être un graphe de carte. On en déduit le résultat algorithmique suivant
...
\begin{theorem}
    ...
    interdits décrits plus tôt présent dans $G$ si ce dernier n'est pas de carte, soit un témoin de $G$ si ce dernier l'est.
\end{theorem}

\begin{proof}
    On suppose sans perte de généralité $G$ $2$-connexe.\\
    On distingue selon si $G$ est $3$-connexe ou non. Dans le premier, l'algorithme renvoyant un sous graphe interdit est presque exactement
    décrit par les preuves du Théorème \ref{cograph3conn} où l'on peut remplacer chaque "il existe un descendant" par une recherche dans
    un sous arbre. Pour l'algorithme renvoyant un témoin, on distingue d'abord le type de $G$ selon le nombre de fils $0$ de la
    racine et puis on utilise les cartes données pour construire un témoin.
    \\~\\
    Dans le deuxième, on décompose d'abord $G$ en composantes $3$-connexes selon son unique séparateur minimal. On vérifie alors
    le type des composantes $3$-connexes en leur trouvant un coarbre à chacune, selon la forme de ce séparateur. Si l'une d'elle
    n'est pas un graphe de carte, on renvoie l'obstruction trouvée. Si elles le sont toutes mais qu'une d'entre elle est d'un
    type trop élevé d'après le Théorème \ref{cographe2conn}, on renvoie l'obstruction décrite en suivant pour cela la preuve du
    Théorème. Pour la construction de témoin, on utilise les cartes décrites afin de les construire.
\end{proof}
...
\begin{proof}
    ...
    de $U$ tels que tous leurs voisins soient parmi $a_1, ..., a_5$, et des sommets de $U$ de degré $2$ tels que l'un des deux voisins
    est parmi $a_1, ..., a_5$.\\
